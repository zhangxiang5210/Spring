...
<body>
   <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 前台管理 <span class="c-gray en">&gt;</span> 预约信息 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
   <div class="col-md-12">
	  <div class="box-info ">
	        <form  action="${pageContext.request.contextPath}/user/order_bill_info.htm" class="form-horizontal" id="form" method="post">
	          ...
	          <div class="box-body">
	            ...
	            <div class="form-group">
	              <label for="memo" class="col-sm-2 control-label">时间(起)</label>
	              <div class="col-sm-4">
	                <input type="text" class="form-control" name="startDate" id="startDate" placeholder="请输入起始预约时间" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss',max:$('#endDate').val()})">
	              </div>
	              <label for="memo" class="col-sm-2 control-label">时间(终)</label>
	              <div class="col-sm-4">
	                <input type="text" class="form-control" name="endDate" id="endDate" placeholder="请输入终止预约时间" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss',min:$('#startDate').val()})">
	              </div>
	            </div>
	            <div class="form-group">
...
function submitForm(){
	...
	$.ajax({
    	type: "POST",
    	url: "${pageContext.request.contextPath}/user/order_bill_info.htm",
    	cache: false,
    	dataType: "json",
    	data:$('#form').serialize(),
